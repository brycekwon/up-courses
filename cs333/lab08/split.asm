	...
	################################################################
	# listSplit -- 	# splits a list into two
	#   $a0: (possibly null) pointer to list's first element
	#   $a1: number of elements to leave in the first list
	# upon return:
	#   $v0 contains a (possibly null) pointer to the list's
	#     first K elements (where K was passed in $a1)
	#   $v1 contains a pointer to the rest of the list
	# Notes:
	#  - if K < 0, K is treated as 0
	#  - if > the list length, K is treated as the list length
	################################################################
listSplit:
    # set initially $v0 = first index of list, $v1 = NULL
	move $v0, $a0
	move $v1, $zero

loop:
    # check if current index is split location
	addi $a1, $a1, -1
	beq $a1, $zero, split
    
    # check if end of the list has been reached
    # $v0 should contain whole list if split location has not been reached
	lw $t1, 4($a0)
	beq $t1, $zero, stop
	lw $a0, 4($a0)

    # continue looping until either condition has been met
	j loop

split:
    # set $v1 to current index and break linked list
    lw $v1, 4($a0)
	sw $zero, 4($a0)
...
main:	
	...
	jal printString
		...
		.text
	jal readInt
	move $s0,$v0 # down-counter

	# ask user how where the split should be
	la $a0,pleaseSplitSpot
	jal printString
		.data
		pleaseSplitSpot:
		.asciiz "Split-spot: "
	...
	la $a0,secondPartIs
		.data
		secondPartIs:
		.asciiz "-------- second part after split--------\n"
		.text
	jal printString
	move $a0,$s1
	jal printIntList
