	...
	################################################################
	# merge -- merges two sorted lists together into a sorted list	
	#   $a0 - pointer to first sorted list (possibly null)
	#   $a1 - pointer to second sorted list (possibly null)
	# uponn return:
	#   $v0 - the merged/list
	################################################################

merge:
	# dummied up: just return a pointer to the first list
	# move $v0,$a0
...
nonEmptyLists:
    ...
    # stack prolog
    subu $sp, $sp, 16   # open 4 slots
    sw $ra, 0($sp)      # return addr
    sw $s0, 4($sp)
    sw $s1, 8($sp)
    sw $s2, 12($sp)
    
    # save the a-registers into s-registers
    move $s0, $a0
    move $s1, $a1

    # extract and compare the values in the first nodes of the lists
    lw $a0, 0($s0)
    lw $a1, 0($s1)
    ble $a0, $a1, retList0
...
main:	
	...
	# ask user how long the lists should be
	la $a0,pleaseLength1
	jal printString
		.data
		pleaseLength1:
		.asciiz "Length of first list: "
		.text
	jal readInt
	move $s0,$v0
	la $a0,pleaseLength2
	jal printString
		.data
		pleaseLength2:
		.asciiz "Length of second list: "
		.text
	jal readInt
	move $s1,$v0

	# delta-limints for producing increasing random sequence
	li $s3,1
	li $s4,6

	# create first random list of small numbers
	move $a0,$s0
	move $a1,$s3
	move $a2,$s4
	jal makeRandomList
	...
	# accumuate sum on list
	move $a0,$v0
	jal accumSumList	
	move $s0,$v0

	# create second random list of small numbers
	move $a0,$s1
	move $a1,$s3
	move $a2,$s4
	jal makeRandomList
	...
	# print the merged list, and dashes
	la $a0,listM
	jal printString
		.data
		listM:
...
inLoop1:	
	...
	# ================ accumSumList ================
	# sets each element's data to the sum of -8, itself and all
	# the previous elements
	# $a0 - pointer to first list node
accumSumList:	

	# current sum
	li $t0,-8

	# test for null list, just returning if so
	beqz $a0,doneAccumSumList
...
accumSumLoop:
	...
	# go to next element; test for null
	lw $a0,4($a0)	
	bnez $a0,accumSumLoop
