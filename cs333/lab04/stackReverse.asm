...
main:	
	...
	# user input/stack counter = 0
	move $s0, $zero
	
	# loop that reads the integers
loop:	
	...
	# exit loop if it did not read an integer
	and $t0,$v1,0x1
	beqz $t0,print
	...
	# push input into the stack: subtract 4 from the stack
	# pointer ($sp) to move the pointer upward. Then store
	# the input into the stack. 
	subu $sp, $sp, 4
	sw $v0, 0($sp)
	...
	# loop back to read next integer
	b loop

	##### done with reading loop ####

print:
	# exit if there is no more valid inputs
	beqz $s0, exit

	# pop values from the stack: read the value from the
	# stack. Then add 4 to the stack pointer ($sp) to
	...
	# loop back to the next value in the stack
	b print
...
peekChar:
	...
	################################################################
	.data
	# string containing newline character
prompt:
	.asciiz "Please enter a sequence of integers, separated by spaces: "
newline:
	.asciiz "\n"
